...
# Dumping database structure for loan_khmer
CREATE DATABASE IF NOT EXISTS `loan_khmer` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `loan_khmer`;
...
# Dumping data for table loan_khmer.users: ~0 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `user_groups` (
  ...
  `gro_status` bit(1) DEFAULT b'0',
  `user_groupscol` varchar(45) DEFAULT NULL,
  `status` bit(1) DEFAULT NULL,
  PRIMARY KEY (`gro_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

# Dumping data for table loan_khmer.user_groups: ~0 rows (approximately)
