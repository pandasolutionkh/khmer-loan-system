...
//        $query_data = $this->m_global->select_where('loan_account', array('loa_acc_code' => $loan_code));
        $query_data = $this->m_global->select_join('loan_account', array('currency' => array('loa_acc_cur_id' => 'cur_id'),'repayment_schedule' => array('loa_acc_id' => 'rep_sch_loa_acc_id')), 'inner', array('loa_acc_code' => $loan_code,'rep_sch_status'=>NULL), '1');
...
//        echo $query_data;exit();
    ...
    function update() {
        ...
        $update_query = $this->m_global->update('repayment_schedule', array('rep_sch_status' => '1','rep_sch_description'=>$loan_des, 'rep_sch_value_date' => date('y-m-d h:i:s')), array('rep_sch_loa_acc_id' => $loan_id, 'DATE(rep_sch_date_repay)' => $limit_date));
